...
-- FIX: Rewrote makeDraggable. The original had a broken dragStart nil-guard in
-- InputChanged on the drag handle, which would silently overwrite dragStart during
-- a live drag and cause jitter or incorrect delta calculations.
...
-- FIX: ripple now correctly computes position relative to the button's AbsolutePosition
-- rather than relying on a raw GetMouseLocation call which can be offset by GuiInset.
...
local function buttonInteract(btn, callback)
    btn.MouseButton1Click:Connect(function()
        ...
        -- Offset by GuiInset so the ripple sits correctly under the cursor
        local inset = game:GetService("GuiService"):GetGuiInset()
        local relative = p - inset - btn.AbsolutePosition
...
function RaylikeUI:CreateWindow(options)
    ...
    -- FIX: topMask now uses the correct BackgroundColor and properly fills the
    -- lower portion of topBar to square off its bottom edge (corners only needed at top).
    ...
    -- FIX: tabBar now uses AutomaticSize.Y so it doesn't collapse when content grows.
    ...
    -- FIX: Keep tabList CanvasSize in sync with its content
    ...
    -- FIX: closeBtn callback now guards against double-destroy
    ...
    function selfWindow:CreateTab(tabName, iconId)
        ...
        -- FIX: page ScrollingFrame now properly syncs CanvasSize via AutomaticCanvasSize
...
-- FIX: Rainbow used `(t % 5) / 5` which cycles hue over only 1/5 of the spectrum
-- per second. Changed to `t % 1` for a proper full-spectrum rainbow.
function RaylikeUI.Utils.Rainbow(speed, offset)
    local s = speed or 1
    local o = offset or 0
    local t = (os.clock() * s + o) % 1
    return Color3.fromHSV(t, 0.75, 1)
end
...
function RaylikeUI.Utils.ApplyThemeRecursive(root, map)
    for _, obj in ipairs(root:GetDescendants()) do
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            if map.TextColor3 then obj.TextColor3 = map.TextColor3 end
        end
        if obj:IsA("Frame") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            if map.BackgroundColor3 then obj.BackgroundColor3 = map.BackgroundColor3 end
        end
    end
end
...
function RaylikeUI.Utils.TableReduce(t, fn, acc)
    local value = acc
    for i, v in ipairs(t) do value = fn(value, v, i) end
    return value
end
...
-- FIX: Registry methods had `self` as their first param but were never called as
-- methods — the first real argument was silently swallowed into `self`. Removed it.
RaylikeUI.Registry = RaylikeUI.Registry or {}

local function makeRegistryMethod(name)
...
for i = 1, 88 do
    RaylikeUI.Registry["Method" .. i] = makeRegistryMethod("Method" .. i)
end
